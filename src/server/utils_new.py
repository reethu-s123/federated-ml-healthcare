...
def calculate_convergence_criteria(prev_loss: float, curr_loss: float, threshold: float = 0.0001) -> bool:
    """Calculate convergence based on loss change.
    ...
    if prev_loss == 0:
        return False
    loss_change = abs(prev_loss - curr_loss) / prev_loss
